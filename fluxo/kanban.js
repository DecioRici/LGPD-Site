...
function card(r){
 ...
 return '<article class="truck-card'+(alert?' overdue':'')+'"><div class="card-top"><h4>'+esc(r.plate)+'</h4><span class="badge">'+esc(r.operation)+'</span></div>'+
...
function renderBoard(){
 const data=filtered();
 $('#board').innerHTML=STAGES.map((stage,index)=>{const list=data.filter(r=>r.stage===index);return '<section class="column"><header class="column-head"><h3>'+stage+'</h3><span class="count">'+list.length+'</span></header><div class="card-list">'+(list.length?list.map(card).join(''):'<div class="empty"><strong>Etapa vazia</strong><span>Nenhum veículo neste status.</span></div>')+'</div></section>'}).join('');
}
...
$('#export-csv').addEventListener('click',()=>{
 ...
 const a=document.createElement('a');a.href=URL.createObjectURL(new Blob(['\ufeff'+csv],{type:'text/csv;charset=utf-8'}));a.download='fluxo_logistico_'+new Date().toISOString().slice(0,10)+'.csv';a.click();URL.revokeObjectURL(a.href);notify('Arquivo CSV gerado');
});
